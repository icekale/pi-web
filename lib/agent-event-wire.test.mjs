...
test("forwards cumulative message_update usage without the assistant snapshot", () => {
  ...
  const withoutPartial = toClientAgentEvent({
    type: "message_update",
    message: partial,
    usage,
    assistantMessageEvent: { type: "done" },
  });
  assert.deepEqual(withoutPartial.usage, usage);
  assert.equal(Object.hasOwn(withoutPartial, "message"), false);
});
