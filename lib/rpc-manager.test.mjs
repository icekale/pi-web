...
test("RPC session startup injects the hidden inline extensions and the built-in subagent extension", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /installEmbeddedHostCompat\(\)/);
  assert.match(startupSource, /createEmbeddedHostCompatExtension\(\{ cwd: sessionCwd \}\)/);
  ...
  assert.match(startupSource, /createSubagentExtension\(/);
  assert.match(startupSource, /SUBAGENT_CONTROLLER\.extensionRuntime/);
  assert.match(startupSource, /embeddedHost/);
  assert.match(startupSource, /new AgentSessionWrapper\(inner\)/);
  assert.doesNotMatch(startupSource, /subagentRpc/);
  assert.doesNotMatch(startupSource, /resourceLoaderOptions: \{[^}]*eventBus/);
});

test("wrapper keeps no plugin RPC client now that subagents run in-process", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");

  // The built-in controller owns child sessions, so the host-side plugin bridge
  // must not come back as a wrapper field, reload hook, or accessor.
  assert.doesNotMatch(
    source,
    /SubagentRpcClient|subagentRpcClient|getSubagentRpcClient|createSubagentRpcCapture/,
  );
});
...
test("wrapper treats powershell as a coding builtin and emits coalesced ui_prompt events", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const toolsSource = source.slice(
    source.indexOf("function withExtensionTools"),
    source.indexOf("export class AgentSessionWrapper"),
  );
  const uiSource = source.slice(
    source.indexOf("private emitUiPromptLifecycle"),
    source.indexOf("private requestExtensionUi"),
  );

  assert.match(source, /CODING_BUILTIN_TOOLS/);
  assert.match(toolsSource, /const codingToolNames = new Set\(CODING_TOOL_NAMES\)/);
  assert.match(source, /const CODING_TOOL_NAMES = CODING_BUILTIN_TOOLS/);
  assert.match(uiSource, /ui_prompt_start/);
  assert.match(uiSource, /ui_prompt_end/);
  assert.match(uiSource, /this\.uiPromptDepth === 1/);
  assert.match(source, /withUiPrompt\("select"/);
  assert.match(source, /withUiPrompt\("custom"/);
});
...
test("fork_branch copies the selected assistant entry without replacing the source session", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const forkBranch = source.slice(
    source.indexOf('case "fork_branch"'),
    source.indexOf('case "navigate_tree"'),
  );
  assert.match(forkBranch, /createBranchedSession\(entryId\)/);
  assert.doesNotMatch(forkBranch, /await this\.shutdown\(\)/);
});
...
test("set_model and set_thinking_level return the applied thinking level", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const setModel = source.slice(source.indexOf('case "set_model":'), source.indexOf('case "fork":'));
  const setThinking = source.slice(source.indexOf('case "set_thinking_level":'), source.indexOf('case "compact":'));
  assert.match(setModel, /thinkingLevel: this\.inner\.thinkingLevel/);
  assert.match(setThinking, /return \{ level: this\.inner\.thinkingLevel \}/);
});
