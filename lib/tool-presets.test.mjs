...
test("treats powershell as an optional builtin that does not change presets", async () => {
  const { CODING_BUILTIN_TOOLS, OPTIONAL_BUILTIN_TOOLS } = await jiti.import("./tool-presets.ts");
  assert.deepEqual([...OPTIONAL_BUILTIN_TOOLS], ["powershell"]);
  assert.ok(CODING_BUILTIN_TOOLS.includes("powershell"));
  assert.ok(!PRESET_FULL.includes("powershell"));
  assert.ok(!PRESET_DEFAULT.includes("powershell"));
  assert.equal(
    getPresetFromTools(toolEntries([...PRESET_DEFAULT, "powershell"], ["powershell"])),
    "default",
  );
});
