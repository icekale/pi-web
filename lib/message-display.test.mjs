...
test("does not let trailing thinking swallow the final answer", async () => {
  const { splitFinalAssistantBlocks } = await loadSubject();
  const message = assistant([
    { type: "thinking", thinking: "plan" },
    { type: "toolCall", toolCallId: "call-1", toolName: "bash", input: {} },
    { type: "text", text: "Here is the report." },
    { type: "thinking", thinking: "double-check" },
  ]);

  const result = splitFinalAssistantBlocks(message, { isStreaming: false });

  assert.deepEqual(result.answerBlocks.map((block) => block.type), ["text"]);
  assert.equal(result.answerBlocks[0].text, "Here is the report.");
  assert.deepEqual(result.processBlocks.map((block) => block.type), ["thinking", "toolCall", "thinking"]);
});
