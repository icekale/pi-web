...
test("groups consecutive thinking blocks apart from process blocks", async () => {
  const { splitThinkingBlocks } = await loadSubject();
  const groups = splitThinkingBlocks([
  ...
  assert.deepEqual(groups.map((group) => [group.thinking, group.blocks.map((block) => block.type)]), [
