...
test("heartbeats are EventSource data frames the client can observe", async () => {
  const source = await readFile(new URL("./agent-event-stream.ts", import.meta.url), "utf8");
  assert.match(source, /type: "heartbeat"/);
  assert.match(source, /enqueueText\(":\\n\\n"\)/);
  assert.match(source, /setSessionLease/);
  assert.match(source, /hasActiveSessionLease/);
});
