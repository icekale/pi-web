...
test("mergeWindowedHistory drops thinking-only live rows once disk has the answer", () => {
  const thinking = { role: "assistant", content: [{ type: "thinking", thinking: "..." }] };
  ...
  assert.equal(merged.items.length, 2);
  assert.deepEqual(merged.items[1], answer);
});

test("mergeWindowedHistory keeps thinking-only live rows until disk has an answer", () => {
  const thinking = { role: "assistant", content: [{ type: "thinking", thinking: "..." }] };
  const merged = mergeWindowedHistory(
    [{ role: "user", content: "hi" }, thinking],
    ["u1"],
  ...
  assert.equal(merged.items[1], thinking);
});
