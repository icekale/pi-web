...
test("idle timer does not reap a selected session while its lease is live", async (t) => {
  ...
  t.after(() => wrapper.destroy());
  wrapper.start();
  wrapper.setSessionLease(Date.now() + 45_000);

  t.mock.timers.tick(10 * 60 * 1000);
  await nextTurn();

  assert.equal(wrapper.isAlive(), true);
  assert.equal(wrapper.hasActiveSessionLease(), true);
});
...
test("shutting down one session does not emit session_shutdown on a sibling", async () => {
  const leftCalls = [];
  const rightCalls = [];
  const left = new AgentSessionWrapper({
    isBashRunning: false,
    extensionRunner: {
      async emit(event) {
        leftCalls.push(event);
      },
    },
    dispose() {
      leftCalls.push("dispose");
    },
  });
  const right = new AgentSessionWrapper({
    isBashRunning: false,
    extensionRunner: {
      async emit(event) {
        rightCalls.push(event);
      },
    },
    dispose() {
      rightCalls.push("dispose");
    },
  });

  await left.shutdown();

  assert.deepEqual(leftCalls, [
    { type: "session_shutdown", reason: "quit" },
    "dispose",
  ]);
  assert.deepEqual(rightCalls, []);
  assert.equal(left.isAlive(), false);
  assert.equal(right.isAlive(), true);
...
test("replays in-flight tool execution events to a new SSE subscriber", async (t) => {
  let listener;
  const inner = {
    isBashRunning: false,
    isStreaming: true,
  ...
  assert.equal(replayed[0].type, "tool_execution_update");
  assert.equal(replayed[0].toolCallId, "call-1");

  listener({
    type: "tool_execution_end",
    toolCallId: "call-1",
    result: { role: "toolResult", toolCallId: "call-1", toolName: "bash", content: [{ type: "text", text: "done" }], isError: false, timestamp: 1 },
  });
  const afterEnd = [];
  wrapper.onEvent((event) => afterEnd.push(event));
  assert.equal(afterEnd.some((event) => event.toolCallId === "call-1"), false);
});
