...
test("staging never writes .output into the repository", () => {
  ...
  const result = runStage(outputDir, stageDir);
  assert.equal(result.status, 0, result.stderr);
  // The deployed `.output` is a symlink to a build outside the repository, so only a
  // real directory here means staging wrote into the working tree.
  const outputPath = join(ROOT, ".output");
  assert.ok(
    !existsSync(outputPath) || lstatSync(outputPath).isSymbolicLink(),
    "repository must not contain a real .output directory",
  );
});
