...
test("omits implicit off thinkingLevel when the path never set one", () => {
  const context = buildSessionContext([
    userEntry("u1", null, "hello"),
    assistantEntry("a1", "u1", "world"),
  ]);
  assert.equal(context.thinkingLevel, "");
});

test("keeps an explicit thinking_level_change on the active path", () => {
  const context = buildSessionContext([
    userEntry("u1", null, "hello"),
    {
...
test("disk sessions replace runtime snapshots with the same id", () => {
  ...
  assert.equal(merged[1].firstMessage, "persisted");
  assert.equal(merged[1].path, persisted.path);
  assert.equal(merged[1].transient, undefined);
  assert.equal(merged[1].modified, runtime.modified);
});

test("disk modified wins when it is newer than the runtime snapshot", () => {
  const persisted = {
    path: "/tmp/session.jsonl",
    id: "same-id",
    cwd: "/tmp",
...
test("findSessionEntry reads the tail first then earlier prefixes", () => {
  ...
  const window = readSessionWindow(filePath, { limit: 10 });
  assert.equal(window.context.entryIds.at(-1), "late");
  assert.equal(findSessionEntry(filePath, "late")?.id, "late");
});
