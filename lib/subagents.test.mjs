...
// C2-a: a repository ships `.pi/agents/*.md` with its own tools, model, and system
// prompt. Opening an untrusted repository must not let it redefine a built-in profile
// by name, the same way `.pi/skills` and `.pi/extensions` need a trust decision.
test("an untrusted project cannot redefine a built-in profile by name", async () => {
  const { getProjectTrustStatus, trustProject } = await createJiti(import.meta.url).import("./project-trust.ts");
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-untrusted-"));
  try {
    await mkdir(join(cwd, ".pi", "agents"), { recursive: true });
    await mkdir(join(cwd, ".pi", "extensions"), { recursive: true });
    await writeFile(join(cwd, ".pi", "extensions", "probe.js"), "export default () => {};\n");
    await writeFile(
      join(cwd, ".pi", "agents", "Explore.md"),
      "---\ndescription: Repository scout\ntools: read, grep\n---\nRead carefully and report findings.\n",
    );

    assert.deepEqual(getProjectTrustStatus(cwd, testAgentDir), { requiresTrust: true, trusted: false });
    const shadowed = listSubagentProfiles(cwd).find((item) => item.name.toLowerCase() === "explore");
    assert.equal(shadowed.scope, "builtin", "an untrusted repository must not shadow a built-in profile");
    assert.equal(
      listSubagentProfileSources(cwd).some((item) => item.scope === "project"),
      false,
      "untrusted repository profiles must not be listed at all",
    );

    // The same repository profile takes effect once the visitor trusts the project.
    trustProject(cwd, testAgentDir);
    const trusted = listSubagentProfiles(cwd).find((item) => item.name.toLowerCase() === "explore");
    assert.equal(trusted.scope, "project");
    assert.equal(trusted.description, "Repository scout");
