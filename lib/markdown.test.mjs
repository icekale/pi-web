...
describe("normalizeDisplayMath", () => {
  ...
  describe("\\[ … \\] blocks", () => {
    ...
    it("normalizes loose one-line brackets that look like math", () => {
      assert.equal(normalizeDisplayMath("[\\frac{1}{2}]"), "$$\n\\frac{1}{2}\n$$");
    });

    it("leaves ordinary one-line brackets alone", () => {
      assert.equal(normalizeDisplayMath("[release notes]"), "[release notes]");
    });
  });
});
