...
const EXPECTED_ROUTES = {
  ...
  "app/api/providers/[provider]/usage/route.ts": ["GET"],
  ...
  "app/api/subagents/route.ts": ["GET", "PUT"],
...
const EXPECTED_ADAPTERS = {
  ...
  "src/routes/api/providers/$provider/usage.ts": { route: "/api/providers/$provider/usage", methods: ["GET"] },
  ...
  "src/routes/api/subagents.ts": { route: "/api/subagents", methods: ["GET", "PUT"] },
...
/**
 * `app/api/sessions/[id]/route.ts` → `/api/sessions/$id`, and the catch-all
 * `app/api/files/[...path]/route.ts` → `/api/files/$`.
 */
function adapterRouteFor(routeFile) {
...
test("every API handler has a thin TanStack adapter with the expected route and methods", async () => {
  assert.equal(Object.keys(EXPECTED_ADAPTERS).length, 47);
...
test("every internal API route has an adapter and a runtime method entry", () => {
  const adapterRoutes = new Map(Object.values(EXPECTED_ADAPTERS).map((entry) => [entry.route, entry.methods]));
  for (const [file, methods] of Object.entries(EXPECTED_ROUTES)) {
    const route = adapterRouteFor(file);
    const adapter = adapterRoutes.get(route);
    assert.ok(adapter, `${file} → ${route} has no TanStack adapter`);
    assert.deepEqual([...adapter].sort(), [...methods].sort(), `${route} methods`);
  }
  assert.equal(adapterRoutes.size, Object.keys(EXPECTED_ROUTES).length);
});
...
test("the internal API inventory contains exactly the expected 47 routes", async () => {
