...
test("get_subagent_result covers missing, running, completed, failed, and aborted runs", async () => {
  ...
  result = await tool.execute("call", { agent_id: "child-session" });
  assert.equal(result.content[0].text, "Subagent child-session is queued.");
...
test("get_subagent_result waits through the queued state instead of reporting a failure", async () => {
  let calls = 0;
  const { tools } = await loadTools({
    async start() { throw new Error("unused"); },
    async get() {
      calls += 1;
      return calls === 1 ? run({ status: "queued" }) : run({ status: "completed", result: "queued then done" });
    },
    async steer() {},
  });
  const result = await tools.get("get_subagent_result").execute("call", { agent_id: "child-session", wait: true });
  assert.equal(result.content[0].text, "queued then done");
  assert.ok(calls >= 2, "the wait loop must re-read a queued run instead of returning immediately");
});
