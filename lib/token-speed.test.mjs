...
test("billed output uses usage.output when it already includes reasoning", () => {
  assert.equal(billedOutputTokens({ output: 1617, reasoning: 1493 }), 1617);
});

test("billed output adds reasoning when it is billed separately", () => {
  assert.equal(billedOutputTokens({ output: 124, reasoning: 1493 }), 1617);
});

test("billed output ignores a zero reasoning field", () => {
  assert.equal(billedOutputTokens({ output: 821, reasoning: 0 }), 821);
});
